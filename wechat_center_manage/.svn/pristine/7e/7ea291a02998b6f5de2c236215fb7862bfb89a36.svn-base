...
<html>
  <head>
	...
	<script type="text/javascript" src="<%=urlPath %>resources/scripts/templatemsg/send_template_msg.js"/></script>
  	...
  	<div style="width:100%; text-align:left;margin-top:20px;margin-bottom:20px;">
        ...
        <table cellspacing="0" cellpadding="0">
            <tr>
                ...
                <c:forEach items="${templateMsgInstances}" var="str">
                   <option value="${str.id}">${str.title}</option>
                </c:forEach>
                </select>
                </c:otherwise>
                </c:choose>
