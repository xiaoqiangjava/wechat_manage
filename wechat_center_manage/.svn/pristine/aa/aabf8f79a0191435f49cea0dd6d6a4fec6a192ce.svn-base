...
<body>
	...
	<div class="gonggao" style="height: auto !important; padding-bottom: 50px;">
		...
		<a height="32" width="106" id="addMaterialText" class="btn btn_primary addMenu" data-gid="">添加菜单</a>
						...
						<td colspan="8" style="padding: 0 10px;font-size: 13px;color:#666;">
						...
						<table class="ListMenu" border="0" cellspacing="0" cellpadding="0" width="1050px">
						<thead>
							<tr>
								<th width="80px">显示顺序</th>
								<th width="150px">主菜单名称</th>
								<th>URL/触发关键词</th>
						...
						<tbody>
							<c:forEach items="${pMenuList}" var="pmenu">
								...
								<c:forEach items="${pMenuList}" var="cmenu">
									<c:if test="${pmenu.menuname eq cmenu.parentmenu and cmenu.menulevel eq 2}">
									<tr>
							...
							<tr>
								<td colspan="8">
									<a height="32" width="106" id="addMaterialText" class="btn btn_primary" onclick="do_create_menu()">发布菜单</a>
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<span id="do_init_menu" style="font-size: 13px;color:red"></span>
								</td>
							</tr>
						</tbody>
						</table>
        	...
        	<input type="hidden" name="pname" value="${currentPublicInfo.pname}" />
				<table border="0" cellpadding="0" cellspacing="0" width="100%" style="font-size: 14px;">
					...
					<tr>
						...
						<td>
							<div class="mr15 l">
							<select name="parentmenu" id="id_parentmenu" class="px" >
					...
					<tr>
						...
						<td><div class="mr15 l">
							<select name="menutype" id="menutype" class="px">
					...
					<tr>
						...
						<td>
							<div class="mr15 l">&nbsp;
							<label><input type="radio" name="menustatus" checked="checked"  value="0"  />是&nbsp;</label> &emsp;
							<label><input type="radio" name="menustatus"  value="1"  />否&nbsp;</label>
							</div>
							<div class="system l"></div>
						</td>
					</tr>
					<!--<tr class="id_bindstatus_tr">
						<td align="right" height="42" width="110">绑定:</td>
						<td>
							<div class="mr15 l">&nbsp;
							<label><input type="radio" name="bindstatus" checked="checked"  value="10"  />否&nbsp;</label> &emsp;
							<label><input type="radio" name="bindstatus"   value="20"  />是&nbsp;</label>
							</div>
							<div class="system l"></div>
						</td>
					</tr>
					--><tr>
						...
						<td>
							<div class="mr15 l">
							<input id="menuseq" class="px" name="menuseq" title="排序"  value="1" type="text" /></div>
							<div class="system l"></div>
						</td>
					</tr>
					<tr>
					...
					<td></td>
					<td height="50" id="message_check_data" style="color:red;padding-left: 15px;"></td></tr>
				</tbody></table>
				</form>
        </div>
    </div>
...
$("#message_check_data").html("");
	...
	$(".addMenu").click(function(){
		$(".dialog-content-").css({display:"block",height:$(document).height()});
		  ...
		  $(".dialog-content").css("top",yscroll+100);
	...
	$("#menutype").change(function(){
		...
		if(menutype == 'click'){
			$("#menutype_value").html('<input id="menukey" class="px" style="width:150px" name="menukey" title="触发关键词" value="" type="text" /> （填写关键词）');
		}else if(menutype == 'view'){
			$("#menutype_value").html('<input id="menuurl" class="px" style="width:150px" name="menuurl" title="触发URL" value="" type="text" /> （填写触发URL）');
		}else{
	...
	//是否绑定	
/***$("#id_parentmenu").change(function(){
		var pmVal = $("#id_parentmenu").val();
		if(pmVal == 0){
			$(".id_bindstatus_tr").css("display","none");
		}else{
			$(".id_bindstatus_tr").css("display","table-row");
		}
	}); ***/
	...
	$.ajax({
		...
		success:function(data){
			if(data.success){
				...
				$("#do_init_menu").html("（菜单发布失败，请检查数据格式和网络状态）");
			}
		},
		error:function(data){
