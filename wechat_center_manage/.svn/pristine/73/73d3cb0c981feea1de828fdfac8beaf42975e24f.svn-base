  ...
  <body>
  	<div class="wxLayout">
        ...
        <table cellspacing="0" cellpadding="0">
            <tr>
                <td colspan="7">模板管理</td>
            </tr>
            <tr>
            	<td width="5%">ID</td>
                <td width="19%">模板ID</td>
                <td width="19%">名称</td>
                <td width="19%">first值</td>
                <!--  
                <td width="11%">keyword1值</td>
                <td width="11%">keyword2值</td>
                <td width="11%">keyword3值</td>
                <td width="11%">keyword4值</td>
                -->
                <td width="19%">remark值</td>
                <td width="19%" colspan="2">操作</td>
            </tr>
            <c:choose>
            <c:when test="${empty templateMsgInstances}">
            <h3 style="color:red">还未添加任何消息模板</h3>
            </c:when>
            <c:otherwise>
            <c:forEach items="${templateMsgInstances}" var="str" varStatus="status">
            <tr>
            	<td>${str.id}</td>
                ...
                -->
                <td>${str.remark}</td>
                <td><a class="btn" href="index/templatemsg/editInstance?id=${str.id}">修改</a></td>
                <td><a class="btn" href="index/templatemsg/deleteInstance/${str.id}" onclick="return(confirm('确定刪除?'))">删除</a></td>
            </tr>
            </c:forEach>
            </c:otherwise>
            </c:choose>
